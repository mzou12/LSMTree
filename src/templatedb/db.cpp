...
{
    std::optional<Value> mmt_value = mmt.get(key);
    if (mmt_value){
        return mmt_value.value();
    }
...
void templatedb::DB::compact(int level) {
    ...
    SSTable new_sstable(new_entries, deduped_tombs, min, max, new_entries.size()+deduped_tombs.size(), start_seq);
