...
std::vector<Fragment> build_fragments(const std::vector<RangeTomb>& tombs) {
    // 1. 收集所有边界
    std::set<int> bounds;
    for (const auto& t : tombs) {
...
bool entry_cmp(const Entry& a, const Entry& b) {
...
bool tomb_cmp(const RangeTomb& a, const RangeTomb& b) {
